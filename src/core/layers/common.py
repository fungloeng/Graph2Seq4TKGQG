...
class EncoderRNN(nn.Module):
    ...
    def forward(self, x, x_len):
        ...
        x = pack_padded_sequence(
            x[indx],
            sorted_x_len.detach().cpu().tolist(),
            batch_first=True,
...
class DecoderRNN(nn.Module):
  def __init__(self, vocab_size, embed_size, hidden_size, *, rnn_type='lstm', enc_attn=True, dec_attn=True,
    ...
    if enc_attn:
      ...
      self.enc_attn_fn = Attention(self.hidden_size, 2 * self.hidden_size, enc_hidden_size, attn_type='add')
  ...
  def forward(self, embedded, rnn_state, encoder_hiddens=None, decoder_hiddens=None, coverage_vector=None, *,
              input_mask=None, input_node_mask=None, encoder_word_idx=None, ext_vocab_size: int=None, log_prob: bool=True, prev_enc_context=None):
    """
    ...
    if self.enc_attn or self.pointer:
      ...
      enc_energy = self.enc_attn_fn(hidden, encoder_hiddens.transpose(0, 1).contiguous(), \
                          attn_mask=None, addition_vec=addition_vec)
