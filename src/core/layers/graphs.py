...
class GraphNN(nn.Module):
    ...
    def static_graph_update(self, node_state, edge_vec, adj, node_mask=None, ans_state=None):
        ...
        node2edge = to_cuda(torch.stack([torch.from_numpy(x.toarray()).float() for x in node2edge], dim=0), self.device)
        # Shape: (batch_size, num_entities, num_edges)
        edge2node = to_cuda(torch.stack([torch.from_numpy(x.toarray()).float() for x in edge2node], dim=0), self.device)
